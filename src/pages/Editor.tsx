...
export const Editor = () => {
  ...
  const previousSetupRef = useRef({
    ...
    chapter: (lesson || fallback).chapter || (lesson || fallback).topic
  });
  const values = form.watch();
  const currentChapter = values.chapter || values.topic || "";
  const weeklyPlan = isAutoGeneratedWeeklyPlan(values.weeklyPlan, values.subject, values.gradeClass, currentChapter)
    ? createFlexibleWeeklyPlan(values.subject, values.gradeClass, values.chapter || values.topic)
    : values.weeklyPlan || createFlexibleWeeklyPlan(values.subject, values.gradeClass, values.chapter || values.topic);
  ...
  useEffect(() => {
    ...
    if (planCanSync) {
      ...
      if (!current.topic && chapter) {
        form.setValue("topic", `Weekly Lesson Plan - ${chapter}`, {
          shouldDirty: true,
          shouldValidate: false
        });
      }
  ...
  }, [form, values.subject, values.gradeClass, values.chapter, values.topic]);
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_420px]">
        <form className="space-y-4" onSubmit={form.handleSubmit(() => save(true))}>
          ...
          <Card className="p-4">
            <div className="mb-4">
              <h2 className="text-lg font-black text-white">Auto-generated teaching support</h2>
              <p className="text-sm text-muted-foreground">Generated from the subject, grade and chapter.</p>
            </div>
            <div className="grid gap-3 md:grid-cols-3">
              <AutoSupportBox title="Objectives" value={printableLesson.learningObjectives?.map((item) => item.value).filter(Boolean).join("\n") || ""} />
              <AutoSupportBox title="Resources" value={printableLesson.materialsResources?.map((item) => item.value).filter(Boolean).join("; ") || ""} />
              <AutoSupportBox title="Assessment" value={printableLesson.assessment?.teacherComments || ""} />
            </div>
          </Card>
...
const toPrintableLesson = (lesson: LessonPlan): LessonPlan => {
  ...
  const weeklyPlan = isAutoGeneratedWeeklyPlan(lesson.weeklyPlan, subject, gradeClass, chapter)
    ? createFlexibleWeeklyPlan(subject, gradeClass, chapter)
    : lesson.weeklyPlan || createFlexibleWeeklyPlan(subject, gradeClass, chapter);
...
const AutoSupportBox = ({ title, value }: { title: string; value: string }) => (
  <div className="min-h-32 rounded-lg border border-cyan-300/15 bg-[#030d14]/70 p-3">
    <p className="mb-2 text-xs font-black uppercase text-cyan-100">{title}</p>
    <p className="whitespace-pre-line text-sm leading-relaxed text-white/85">{value}</p>
  </div>
);
